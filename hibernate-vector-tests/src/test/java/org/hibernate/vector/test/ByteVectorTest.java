...
public class ByteVectorTest {
	...
	public void testCast(SessionFactoryScope scope) {
		scope.inTransaction( em -> {
			final Tuple vector = em.createSelectionQuery( "select cast(e.theIpVector as string), cast('[1, 1, 1]' as byte_vector(3)) from VectorEntity e where e.id = 1", Tuple.class )
					.getSingleResult();
	...
	public void testTaxicabDistance(SessionFactoryScope scope) {
		scope.inTransaction( em -> {
			...
			assertEquals( 2, results.size() );
			assertEquals( 2L, results.get( 0 ).get( 0 ) );
			assertEquals( taxicabDistance( V2, vector ), results.get( 0 ).get( 1, double.class ), 0D );
			assertEquals( 1L, results.get( 1 ).get( 0 ) );
			assertEquals( taxicabDistance( V1, vector ), results.get( 1 ).get( 1, double.class ), 0D );
		} );
	}
