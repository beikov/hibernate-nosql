...
public class Float32VectorTest extends FloatVectorTest {
	...
	public void testCast(SessionFactoryScope scope) {
		scope.inTransaction( em -> {
			final Tuple vector = em.createSelectionQuery( "select cast(e.theIpVector as string), cast('[1, 1, 1]' as float_vector(3)) from VectorEntity e where e.id = 1", Tuple.class )
					.getSingleResult();
